...
power of the \kl{protocol} (they might influence the number of rounds), but they
...
strings} of Alice and Bob, respectively, the  \kl{protocol} is run as follows. There
...
end of the \kl{protocol}, the output function is used to determine the value of the
function, based on all messages in  the communication history. We say that the
\kl{protocol} computes a language $L \subseteq \Sigma^*$ if for every two strings
$w_1,w_2$, the output of the \kl{protocol} tells us if the concatenation $w_1w_2$
...
will make it impossible to use this information, since the \kl{protocol} can only
compute \kl{regular language}s, as stated in the \cref{thm:boolean-domain} from the
...
\begin{proof}
  We begin with the easier right-to-left implication, which says that the \kl{protocol} can compute every \kl{regular language}. If the language is  \kl(language){regular}, then it  is recognised by a deterministic finite automaton, say  with state space $Q$. To compute the language, we can use a  \kl{one-round protocol} (as we will see in a moment, this is not a coincidence, since all \kl{protocol}s can be reduced to one round). Alice sends the state in $Q$ of the automaton after reading her local string, and Bob sends the dependency $Q \to \set{\text{yes, no}}$ which says how Alice's state determines acceptance. Once these two pieces of information are known, we can apply the function from Bob's message to the state in Alice's message to determine the output.

  The rest of this proof is devoted to the left-to-right implication, i.e.~to showing that every language computed by the \kl{protocol} is \kl(language){regular}. This is the heart of the proof, and the place where we need to tame the non-uniformity of the \kl{protocol}. 
  
  Observe that in the right-to-left implication, we have proved that \kl{regular language}s need only one round. Therefore, if we want to  prove that the \kl{protocol} can only compute \kl{regular language}s, then a by-product will be that the \kl{protocol} collapses to the \kl{one-round} case.  This is, in fact, the first step of our proof. 
  \begin{lemma}\label{lem:one-round-reduction-boolean}
    For every \kl{protocol}, there is an equivalent \kl{one-round protocol}. 
  \end{lemma}
  \begin{proof}
    ...
    (Q_B)^k \to (Q_A)^k.
    \end{align*}
    Instead of waiting for Bob's messages, Alice sends this function. (This function is not a general function, since it must satisfy the following \intro{causality} constraint: the $i$-th coordinate of the output depends only on the first $i-1$ input coordinates.) At the same time, Bob sends an analogous function of type 
    \begin{align*}
    (Q_A)^k \to (Q_B)^k,
    \end{align*}
    which describes the dependency of his messages. Due to the \kl{causality} constraints, the two functions combine to create a unique output in $(Q_A \times Q_B)^k$, which can be used to determine the output of the \kl{protocol}.
  \end{proof}

The proof of the above lemma can incur an exponential cost in the size of the \kl{message space}s. This is of little concern to us, since we only care about the \kl{protocol} having a constant \kl{number of rounds}.
