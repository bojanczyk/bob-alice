...
\section{Streaming String Transducers and Alice-Bob}

\begin{definition}
  A \emph{concatenating Alice-Bob communication model}
  is an Alice-Bob protocol where 
  \begin{enumerate}
    \item Alice and Bob exchange information,
    \item The output is produced by concatenating one 
          word produced by Alice, and 
          one word produced by Bob.
  \end{enumerate}
  Formally, a function $f \colon \Sigma^* \to \Gamma^*$
  is computable in the homomorphism Alice-Bob communication model
  if there exist two functions
  $f_A \colon \Sigma^* \to \Gamma^*$ and
  $f_B \colon \Sigma^* \to \Gamma^*$,
  a finite set $Q$ of signals, and
  two functions
  $q_A \colon \Sigma^* \to Q$
  and
  $q_B \colon \Sigma^* \to Q$,
  such that for all $u,v \in \Sigma^*$,
  \[
    f(uv) = f_A(u, q_B(v)) \; f_B(v, q_A(u)).
  \]
\end{definition}
...
\begin{lemma}
  ...
  Every function $f \colon \Sigma^* \to \Gamma^*$
  computable by a rational function (bimachine)
...
Let us now consider a function $f$ computable by a concatenating Alice-Bob
communication model, so that $f(uv) = f_A(u, q_B(v)) \; f_B(v, q_A(u))$ for
suitable functions $f_A, f_B, q_A, q_B$ and finite set $Q$ of signals. Let us
define a \emph{configuration of Alice} on a word $u \in \Sigma^*$ as the
following tuple: the state $q_A(u)$, and the words $(f_A(u, s))_{s \in Q}$. Let
us write $C(u)$ for this configuration, which defines a map from $\Sigma^*$ to
$Q \times (\Gamma^*)^{|Q|}$.
Let us order configurations as follows: $C(u) \leq C(v)$ if and only if $q_A(u)
...
\end{theorem}
\begin{proof}
  The direction from SSST to concatenating Alice-Bob communication model is
  straightforward, since SSTs correspond to rational functions 
  ...
  Alice-Bob communication model. We are going to construct an SSST computing
  ...
  correctly. We will actually prove a stronger result: $\delta(\varepsilon, w)
  = u$ implies that for every $s \in Q$, the register corresponding to $s$
  equals $f_A(w, s)$, and $q_A(w) = q_A(u)$.

  To prove this, let us write $R(w)_q$ for the content of the register
  corresponding to $q \in Q$ after reading $w$.

  For the base case $w = \varepsilon$, we have $\delta(\varepsilon,
  \varepsilon) = \varepsilon$, and the registers are initialized correctly.
  Now, let us consider a word $w a$ where $a \in \Sigma$ and $w \in \Sigma^*$.
  By induction hypothesis, let us write $u = \delta(\varepsilon, w)$.

  Let us take any $s \in Q$. By definition of 
  the update function, there exists $r \in Q$ such that
  $R(wa)_s = R(w)_r \; w_{u,a,s,v,r}$. By induction hypothesis, we have
  $R(w)_r = f_A(w, r)$, and by definition of $w_{u,a,s,v,r}$, we have
  $f_A(wa, s) = f_A(v, r) \; w_{u,a,s,v,r}$. 
  Hence, we have $R(wa)_s = f_A(wa, s)$, as desired.
  Let us now prove that $q_A(wa) = q_A(v)$.
  By induction hypothesis, we have $q_A(w) = q_A(u)$, and 
  since $C(v) \leq C(ua)$, we have $q_A(ua) = q_A(v)$, as desired.


  Finally, when the SSST is in state $u$ after reading $w$, by what we have
  just proven, we have that the register corresponding to $q_B(\varepsilon)$
  contains $f_A(w, q_B(\varepsilon))$, and since $q_A(w) = q_A(u)$, the output
  function produces $f_A(w, q_B(\varepsilon)) \; f_B(\varepsilon, q_A(w)) =
  f(w)$, as desired.
\end{proof}


As a corollary, we obtain that functions computable by concatenating
Alice-Bob communication models are exactly the rational functions.
