...
\[\text{Output}(x,y) = f_A(x, q_B(y)) \ast f_B(y, q_A(x))\] \footnote{This reminds us of the \emph{Yin-Yang}, so we would like to call this the Yin-Yang protocols. This is because if you consider the local output of Alice as the Yang, then inside it there is the signal from Bob, which is the Yin. Similarly, the local output of Bob is the Yin, which contains the signal from Alice, which is the Yang. Together they form a complete whole.}
