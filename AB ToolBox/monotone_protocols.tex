...
\begin{proof}
    ...
    Now let $ u,v \in \Sigma^* $ such that $ u \RS v $. Then, there exists $ c \in \mathbb N $ (which is only dependent on $ u $ and $ v $) such that for any $ w \in \Sigma *, \norm{f(wu),f(wv)} \leq c $. Therefore we have:
    \begin{align*}
      \norm{f_A[q_B(u)](w).f_B[q_A(w)](u),f_A[q_B(v)](w).f_B[q_A(w)](v)} & \leq c \\
    \end{align*}
