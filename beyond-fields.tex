...
\AP
In our discussion so far, a prominent role was played by \kl{weighted automata} over
...
\AP We begin by recalling the definition of \kl{weighted automaton} in the case
...
However, for the purposes of this section, it  will be more convenient to work
...
\begin{definition}
    ...
    \AP
    A \intro[wa-nfa]{weighted automaton} consists of a finite input alphabet $\Sigma$,  a finite set of states $Q$, and functions: 
    \begin{align}
...
\kl{string-to-string function} is \kl[regular function]{function} if and only if it is
\kl{field continuous}. One could consider variants of this conjecture for
semirings.  For a class $\algclass$ of semirings, let us define
\intro{continuity over $\algclass$} to be the same as in
...
in blue, corresponds to the strongest continuity condition, namely over all
semirings. As we explain in Section~\ref{sec:rational-functions}, this
condition characterises \kl{rational functions}, a well-known transducer class.
The middle part, in red, corresponds to the continuity condition over fields,
which is conjectured to characterise \kl{regular functions}. Finally, the lower
...
\kl{Boolean continuity}, and it is strictly more general than
\kl[regular function]{regularity}. There is no machine model for the black part,
...
the \kl{regular functions}.

\subsubsection{Rational functions}
\label{sec:rational-functions}
\AP
In this section we describe the \kl{rational string-to-string functions}, and
prove that they are exactly the functions which are \kl[continuous@alg]{continuous} over all
...
14.2]{bojanczyk_automata_2025}. The underlying model is \intro{(one-way)
nondeterministic automata with output}. This is like an \textsc{nfa}, except
...
it is done in a \kl[wa-nfa]{weighted automaton} from
...
output string, then it is called a \intro{rational string-to-string function}.
...
\AP
Below we show that: (1) the \kl{rational string-to-string functions} are a
strict subset of the \kl[regular function]{function} ones; and (2) they are exactly the
functions that are \kl[continuous@alg]{continuous} over all semirings. Both results are
straightforward and could be called folklore.

\begin{myexample}
    [Rational is weaker than regular]
    For the separation between \kl[rational function]{function} and 
    \kl[regular function]{function}, a short argument is given in 
    \cite[p. 218]{engelfrietMSODefinableString2001}. 
    The duplicating function $w \mapsto ww$ is \kl[regular function]{function}.
    It cannot be \kl[rational function]{function} 
...
\begin{theorem}\label{thm:rational-functions}
  \AP
  A \kl{string-to-string function} is \kl[rational function]{rational} if and only if it is 
  \kl[continuous@alg]{continuous} over all semirings.
\end{theorem}
\begin{proof}
    ...
    presentation of \kl[wa-nfa]{weighted automata} from
    Definition~\ref{def:weighted-automaton-nondeterministic}. The right-to-left
    implication is proved by viewing a \kl{rational string-to-string function} as a
    special case of \kl{weighted automata}, for suitable semiring. The semiring is 
    \begin{align*}
    \domain = \pfin(\Gamma^*),
    \end{align*}
    ...
    definitions, one sees that a string-to-string relation is \kl[regular function]{function}
    if and only if it is computed by a \kl[wa-nfa]{weighted automaton} over this semiring. 

    This observation gives us the right-to-left implication in the theorem.
    Indeed, suppose that a \kl{string-to-string function} $f : \Sigma^* \to
    \Gamma^*$ is \kl[continuous@alg]{continuous} over all semirings. 
    Consider the \kl[wa-nfa]{weighted automaton}
    \begin{align*}
    \iota : \Gamma^* \to \pfin(\Gamma^*),
    \end{align*}
    which maps a string to itself (as a singleton set). By \kl[continuous@alg]{continuity}, we know
    that $f; \iota$ is computed by a \kl[wa-nfa]{weighted automaton} over $\domain$. This
    ...
    also functional, it follows that $f$ is a \kl{rational function}.
\end{proof}
