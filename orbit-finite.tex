...
\begin{myexample}[Repetitions cannot be detected]
    ...
    appears in Alice's part of the input string, but is not sent as a \kl{message},
    ...
    Alice's part of the string (but not Bob's), then the \kl{communication history}
...
of \kl{Boolean protocols} from Section~\ref{sec:protocols-infinite-alphabet}, and
...
Moreover, none of these models is equivalent to \kl{orbit-finite protocols}:
...
\begin{myexample}
  ...
  \kl{deterministic orbit-finite automata}. One reason is that protocols are
...
\begin{myexample}
    ...
    \ref{ex:some-twice}, can be recognised by a \kl{nondeterministic orbit-finite
    automaton}, but cannot be computed by a \kl(orbit-finite){protocol}.
    We do not know yet if
    \kl{nondeterministic orbit-finite automata} can simulate all \kl(orbit-finite){protocols}, 
    and therefore for
    ...
    of \kl{nondeterministic orbit-finite automata} -- see
...
automata} are closed under complement, since \kl(orbit-finite){protocols} can
...
\begin{definition}
    [Vector space of orbit-finite dimension]\label{def:orbit-finite-vector-space} A vector space of orbit-finite dimension is a vector space of the form $\lincomb Q$ for some orbit-finite set $Q$.
\end{definition}
