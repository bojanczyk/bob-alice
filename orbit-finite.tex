...
\begin{myexample}[First letter equals last letter]
    Consider the language in~\eqref{eq:first-last}, i.e.~the language of strings whose first letter is equal to the last letter. This language can be computed by a two-round protocol. In  the first round, Alice sends the first letter of her local string, or the bit $0$ to indicate that her local string is empty. Bob uses this information to check membership in the language. 
\end{myexample}

\begin{myexample}[Some letter appears at least twice]
    \label{ex:some-twice}
    Consider the language in~\eqref{eq:some-twice}, i.e.~the language of strings where some letter appears at least twice. As we will see, this language cannot be computed by a protocol. The reason is that if Alice does not see a repetition in her part of the input string, then she should send all of her letters to Bob, since any of them could be repeated in Bob's part. However, Alice cannot send all of her letters, since there is a constant number of rounds. 
\end{myexample}
...
The second step in the proof of \cref{thm:orbit-finite-protocol-to-weighted} is to show that orbit-finite bilinear protocols are equivalent to weighted orbit-finite automata.  This will be proved similarly to the finite case, see \cref{sec:from-scalar-product-protocol-to-weighted-automaton}, by appealing to the Fliess Theorem. However, we need an orbit-finite version of this theorem, so we need to prove that. 
