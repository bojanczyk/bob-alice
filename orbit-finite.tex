...
reachable subsets of $Q$ might not be \kl{orbit-finite}, and therefore it cannot be sent
...
% This example can be summarized by saying that orbit-finiteness
% is not preserved by the powerset construction. This is one of the main obstacles when working with orbit-finite sets.
% For example, it is the reason why nondeterministic orbit-finite automata are more powerful than deterministic ones.
\end{myexample}
...
\begin{lemma}\label{lem:fixed-atoms}
  ...
  \begin{enumerate}
    \item   every state from $X_1$ contains an \kl{atom} from $T$ on some \kl{free coordinate}; or 
    \item   every state from $X_2$ contains an \kl{atom} from $T$ on some \kl{free coordinate}.
  \end{enumerate}
\end{lemma}
    \begin{proof}
      \AP
...
\begin{claim}\label{claim:sunflower}
  Let $Q$ be an \kl{$S$-orbit} of \kl[orbit dimension]{dimension} $d$, and let $X$ be a subset of $Q$.
  If $X$ does not contain two \kl{$S$-disjoint} elements, then there is a
  set $T$ of at most $d$ atoms such that every element of $X$ uses at least one atom from $T$ on a \kl{free coordinate}.
\end{claim}
\begin{proof}
...
\end{myexample}
%\rs{I don't think this example works -- The protocols cannot check that the input is functional. }
...
\begin{proof}
  \AP
    ...
    \begin{proof}
        ...
        equivariant functions. Before defining this notion formally, let us present an example:

        \begin{myexample}[Partial application not equivariant]
        \label{ex:partial-application-not-equivariant}
        A partial application of a function $f : X \times Y \to Z$ to an argument $x \in X$ is a function $f(x, -) : Y \to Z$, defined as $f(x, -)(y) = f(x,y)$. An important observation is that even if $f$ is equivariant, the partially applied function $f(x, -)$ might not be equivariant. For example, consider
        \begin{align*}
        f: \eqfun{\atoms^* \times \atoms} {\set{\textrm{true}, \textrm{false}}},
        \end{align*}
        ...
        \end{align*}
        which is of type $\atoms \to \set{\textrm{true}, \textrm{false}}$. Observe that this function is not equivariant:
        If we take an atom permutation $\pi$ that swaps $\textrm{John}$ with $\textrm{Eve}$, then we get that
        \begin{align*}
          f'(\pi(\textrm{John})) = f'(\textrm{Eve}) = \textrm{false} \qquad \text{but} \qquad \pi(f'(\textrm{John})) = \pi(\textrm{true}) = \textrm{true}.
        \end{align*}
        However, intuitively $f'$ is ``almost equivariant'', since it only depends on the atoms Celine, John, and Anne.
        This is formalized below using as finitely-supported functions. 
      \end{myexample}
      \begin{definition} [Finitely-supported function]
        \label{def:finitely-supported-function}
        \AP
         A function $f : X \to Y$ is \intro{finitely-supported} if there is a finite set $S$ of atoms such that for every atom permutation $\pi$
         that fixes all atoms from $S$ (i.e. $\forall_{s \in S}, \pi(s) = s$), we have 
        \begin{align*}
        \forall_{x \in X} \qquad f(\pi(x)) = \pi(f(x))
        \end{align*}
        %The set of all finitely-supported functions from $X$ to $Y$ is denoted by $\fsfun{X}{Y}$.
      \end{definition}
      An important property of finitely-supported function spaces is that they do not preserve orbit-finiteness: 

      \begin{myexample}
        The set $\fsfun{X}{Y}$ might not be \kl{orbit-finite}, even if both $X$ and $Y$ are.
        For example, consider the set $\fsfun{\atoms}{\set{\textrm{true},\textrm{false}}}$. For every finite set $S$ of atoms, 
        we can define a finitely-supported function $f_S : \atoms \to \set{\textrm{true},\textrm{false}}$ that maps all atoms from $S$ to true and all other atoms to false.
        Each such function $f_S$ is finitely-supported, as it is supported by $S$. However, since $S$ can be arbitrary large, representing all such 
        function $f_S$ would require an unbounded number of atoms, and therefore cannot be done in an \kl{orbit-finite} way.
      \end{myexample}
      ...
      \begin{lemma}[{\cite[Lemma 8.17]{bojanczyk_slightly}}]
      \label{lem:function-space-of-ofd}
            For all orbit-finite sets $X$ and $Y$, the space of finitely-supported functions $\fsfun{X}{\lincomb Y}$ is a \kl{vector space of orbit-finite dimension}\footnote{
              Actually, the cited lemma states an equivalent result that the space of all 
              finitely-supported linear transformations $\fslinfun{\lincomb X}{\lincomb Y}$ is a \kl{vector space of orbit-finite dimension}. However, the proof in \cite{bojanczyk_slightly} starts by stating
      ...
      and go back to the proof of the claim. Observe, that the second coordinate in~\eqref{eq:contribution-last-two-rounds} is indeed of type $\fsfun{Q_B}{V_A}$,
