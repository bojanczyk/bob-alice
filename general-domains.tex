...
\begin{myexample}
    If the operations are $+$ and $\times$, then after applying the usual distributivity laws,   \kl{term operation}s are the same as polynomials with natural coefficients, such as 
\begin{align*}
x_1x_2^2 + x_1x_2x_3^3  + \myunderbrace{2x_1}{same as $x_1 + x_1$}.
\end{align*}
If the \kl{output domain} is the \kl{Boolean domain}, which has no operations, then the only kind of \kl{term operation} is a single variable, e.g.~$x_2$.  In the case of a \kl{String domain}, a \kl{term operation} is some concatenation of the variables, such as $x_1 x_3 x_1 x_2$.
\end{myexample}



We now generalise the \kl{protocol} to cover functions 
\begin{align*}
f : \Sigma^* \to \domain,
\end{align*}
where $\domain$ is some possibly infinite \kl{output domain}. As in the Boolean
version of the \kl{protocol}, the output value is constructed by Alice and Bob, as a
...
\kl{signal part} consists of a finite amount of information, and is used to exchange
information between the two parties as in the \kl{Boolean protocol}. The \kl{output part}
is a list of elements from the  \kl{output domain}, and  is meant to be part of the
...
contain $d$ elements of the \kl{output domain}. This does not affect the expressive
power of the \kl{protocol}, but it might affect the \kl{number of rounds}, and the above
definition will be more convenient later one, where we consider \kl{one-round protocol}s. Another important difference is that the operations from the \kl{output domain} are only applied once at the end of the \kl{protocol}. This, again does
...
\begin{definition}\label{def:two-party-protocol-general} A \intro{two-party protocol}
  ...
  \begin{enumerate}
    ...
    \end{align*}
    \item a \intro[output function@protocol]{output function} of type \begin{align*}
...
The \kl{protocol} is executed on a pair of strings $(w_1,w_2)$. In each round, each of the two parties  sends a message (which consists of a signal and some elements of the \kl{output domain}) that is based on their \intro{local string} and the history of signals coming from the other party. After all $k$ rounds have been executed, the joint signal history of both players is used, by the \kl[output function@protocol]{output function}, to determine a \kl{term operation}. This operation is then applied to the output history, yielding the final result of the \kl{protocol}. As in the Boolean case, we are interested in \kl{protocol}s that are \intro{split invariant}, which means that for every string $w \in \Sigma^*$, the same output is produced for every possible decomposition $w = w_1 w_2$. Such \kl{protocol}s compute a function of type $\Sigma^* \to \domain$. 
...
We have little say to say about  \kl{protocol}s in the case of a general \kl{output domain}. The  only result that we have at this level of generality is a
reduction to \kl{one-round protocol}s, similarly to the Boolean case.

\begin{lemma}
\label{lemma:one-round-reduction-general}
  Every \kl{protocol} is equivalent to a \kl{one-round protocol}.
\end{lemma}
\begin{proof}
  Same proof as in \cref{lem:one-round-reduction-boolean}. 
\end{proof}

 Later in this paper, we will study \kl{protocol}s for infinite input alphabets. Such \kl{protocol}s will not be a special case of the \kl{protocol}s developped so far, since the \kl{signal space}s will be infinite, albeit in a limited way (called \kl{orbit-finite}).  As we will see, for such \kl{signal space}s the reduction to one round will no longer be valid, and the interactive nature of the \kl{protocol}s will be essential.
