...
\begin{itemize}
    \item \textbf{Boolean domain.} The set is $\set{\text{yes, no}}$, and there are no basic operations. (We could add basic operations, such as the Boolean operations $\lor,\land$ and $\neg$, but this will not affect the expressive power of our protocol, so we choose to have no operations.)
...
\begin{definition}\label{def:two-party-protocol-general} A two-party protocol   
  ...
  \begin{enumerate}
    ...
    (Q_A \times Q_B)^{k} \to (\domain^{2k} \termop \domain).
    \end{align*}
  \end{enumerate}
\end{definition}
