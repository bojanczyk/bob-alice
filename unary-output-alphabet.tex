...
\begin{definition}[Lookahead automaton]\label{def:bimachine}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \omc{a lookahead automaton consists of another lookahead automaton seems cyclic. Maybe just change it to "bimachine" which is the term used in the literature?}
...
Suppose that the main automaton is in state $w \in W$. If the remaining part of the input string is empty, which can be inferred by having an emptiness check in the lookahead automaton, then the automaton outputs $f(\varepsilon)$.
\omc{It should be $f(w)$ instead of $f(\varepsilon)$, because the invariant is $\Aa_w(v) = f(wv)$}
Otherwise, the remaining input string has at least one letter, and it can be decomposed as $av$, where $a$ is its first letter.  By \cref{lem:minimal-configurations}, there is some $w' \in W$ with
\begin{align*}
w' \leq wa.
\end{align*}
