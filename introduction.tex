...
\begin{description}
    \item[Compositionality.] \omc{I don't think "compositionality" is the best name for this. Perhaps something more descriptive, such as "constant information flow."}  If the input is split into parts, then only a constant amount of information  flows between them, as far as the output of the function is concerned.
\end{description}
...
have unrestricted computational power; the goal is to measure information \omc{information flow instead of information ?} and
...
\kl{protocol} and not on the input string. Also, the  output of the computation must
be \intro{split invariant}, which means that the output depends only on the
...
the  \kl{black box restriction} says that the parties cannot read the messages from
the \kl{output domain}, and they can only operate on them using predefined
operations. For example, in the case of numbers, the operations are addition
and multiplication.

The model, which we call \intro{protocol}s, can be applied to any \kl{output domain},
...
needed to represent it).  This seems to indicate that the \kl{split invariance},
together with a  constant number of messages under the \kl{black box restriction},
has unexpected computational consequences. In particular, in all cases that we
have studied,  \kl{protocol}s coincide -- or are conjectured by us to coincide --
with existing automata models, some of which seem unrelated to the \kl{protocol}s,
as explained in the following table: 
\begin{center}
    \begin{tabular}{ll}
    \textbf{Output} & \textbf{Automata Model} \\
    \hline
    \kl[Boolean domain]{Booleans} & Regular languages (\cref{thm:boolean-domain}) \\
    \kl[String domain]{Field}   & Weighted Automata (\cref{thm:field-domain}) \\
    \kl[String domain]{Strings} & Regular functions (\cref{conj:protocol-regular-string-to-string}) \\
    Boolean, but infinite input alphabet & Unambiguous automata (\cref{conj:protocols-unambiguous}) 
\end{tabular}
\end{center}

\omc{This table is inconsistent. In the second column, we have Automata Model, but in the first row, we list regular languages, which are a class of languages, not an automata model. In the second row, we have weighted automata, which is an automata model. Then, we have regular functions, which are a class of functions, followed by unambiguous automata, which again is a computational model. To make the table consistent, it should list: Finite Automata, Weighted Automata, and 2-way Deterministic Finite Transducers.}
...
output alphabet with one letter (\cref{thm:unary-string-to-string}), and in the
case where the strings are considered under an additional \kl{origin
semantics} (\cref{thm:origin-string-to-string}). In the remainder of this
introduction, we give a more detailed review of the four rows in the table. 
...
f : \Sigma^* \to \set{\text{true, false}},
\end{align*}
i.e.~languages. In this case the two parties only exchange bits \omc{I still think it’s better to say that the messages come from a finite set instead of bits.} , and they need to decide if the input string is in the language or not. As mentioned before, the parties have unbounded computational power. For example, the first message sent by Alice could answer  some undecidable question.
...
\begin{restatable}{theorem}{booleandomain}
 ...
 A language $L \subseteq \Sigma^*$ is computed by a \kl[boolean protocol]{protocol} if and only if it is 
 \kl(language){regular}.
\end{restatable}
...
The string-to-string case is of particular interest because, as we have mentioned earlier in the introduction, there is no general agreement as to  which string-to-string functions should be considered ``\kl(function){regular}''. There are numerous automata models to choose from, some of which are summarised in \cref{fig:transducer-classes}, which contains twenty models, grouped by equivalence into three classes. We can exclude the weakest class (the \kl{rational function}s), since it is too weak: it  cannot compute the reverse or duplicate functions from Example~\ref{ex:reverse-duplicate}. We can also exclude the strongest class (the \kl{polyregular function}s), since it is too strong: \kl{polyregular function}s can have superlinear output size, and as we will see in a moment, \kl{protocol}s can only have linear output size. The remaining class from \cref{fig:transducer-classes} is the middle class of functions. One of the models which defines this class is  \intro{deterministic two-way automaton} with output~\cite{shepherdson1959reduction}. We conjecture  that this class     is the correct answer: 

\begin{conjecture}\label{conj:protocol-regular-string-to-string}
    A string-to-string function is computed by a \kl{protocol} iff it is computed by a \kl{deterministic two-way automaton} with output.
\end{conjecture}

The functions computed by \kl{deterministic two-way automaton}s with output are traditionally called ``\kl(function){regular}'', and the above conjecture could be seen as a justification for using this name.
...
\begin{align*}
\text{\kl{protocol}} \quad \impliedby \quad \text{\kl{two-way automaton}},
\end{align*} 
This implication is rather easy, since a \kl{two-way automaton} can be neatly simulated by the repeated interactions of the \kl{protocol}. The content of the conjecture, and the subject of most of our technical results, is the $\implies$ implication. 
Our first argument in favour of  the conjecture is the following result, which shows that functions computed  by \kl{protocol}s have many properties which are known to hold for \kl{regular function}s.
