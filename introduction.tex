...
Here is an example of a simple language that can be computed by a protocol:

\begin{myexample}[First three equal last three]
    \label{ex:three-letters}
The language ``the first three letters are equal to the last three letters'' over an input alphabet $\set{a,b}$ can be computed by the following protocol:
Alice sends her first three letters to Bob (if Alice got less than three letters, she needs to communicate this), and Bob uses this information to determine membership in the language. Alternatively, Bob can send his last three letters, and other variants are possible as well. A similar protocol will work for constants other than three. However, if the constant is turned into a variable, e.g.~we care about words of the form $ww$ where $w$ is of arbitrary length, then there is no protocol
(by \cref{thm:boolean-domain})
\end{myexample}

As mentioned before, the parties have unbounded computational power, which means they their messages
can contain answers to potentially undecidable questions about their parts of the input.
The first result of this paper says that these protocols compute exactly the regular languages, despite this non-uniformity:
...
\begin{restatable}{theorem}{booleandomain}
 ...
 \kl(language){regular}.
\end{restatable}
One implication is immediate: every regular language can be computed by a protocol. Alice can run an automaton on her part of the input and 
send the intermediate state to Bob who can continue the run on his part. The other implication is proved in Section~\ref{sec:boolean-domain} in two steps. 
In the first step, the protocol is reduced to a one-round non-interactive version, where each party independently sends a message with a constant number of bits, and the decision is then made based on these two messages. In the second step, we use Myhill-Nerode theorem to conclude that any language computed by a one-round protocol must necessarily be regular. 
...
\cref{thm:boolean-domain} is not difficult to prove, nor can it be called completely new. However, it serves as an inspiration for other results in this paper,
 concerning other output domains such as fields or strings. Before continuing with those results, let us briefly discuss the related work.
Although we are not aware of an exact precedent for \cref{thm:boolean-domain}, the general idea of studying communication complexity for regular languages is not new in automata theory. This theme is called state complexity, and it is interested  in the number of states needed for a given language in a given automaton model, and how this number is affected by operations on languages or changes in the model. See  Wikipedia~\cite{stateComplexityWiki} for a comprehensive summary with numerous references, or the recent paper~\cite{goosKiefer2022} which shows how to transfer lower bounds from communication complexity to state complexity of unambiguous automata. 
...
\end{align*}
We adapt the  protocol model to  compute such functions. Similarly to the Boolean case, the parties exchange messages. However, this time there are two kinds of messages: bits (as in the Boolean case), and  elements of the field.  The elements of the field can be added and multiplied.  (Division is discussed in Example~\ref{ex:division}.)  Using bit messages, we can still recognise all regular languages (or more formally their characteristic functions).
